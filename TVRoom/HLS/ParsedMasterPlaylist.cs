...
{
    public sealed class ParsedMasterPlaylist
    {
        public int HlsVersion { get; }
        public string StreamInfo { get; }

        private ParsedMasterPlaylist(int hlsVersion, string streamInfo)
        {
            HlsVersion = hlsVersion;
            StreamInfo = streamInfo;
        }

        public static bool TryParse(ReadOnlySpan<byte> payload, [MaybeNullWhen(false)] out ParsedMasterPlaylist parsed)
        {
            ...
            {
                parsed = new ParsedMasterPlaylist(version, streamInfo);
                return true;
            }
